...
3.线上onlineTest
    ...
    test  BCD->E
    test  CDE->F
4.使用ExtraTreesRegressor 连续预测7天数值
5.第二星期利用预测出来的第一星期的结果进行预测
6.如果先对第一周预测出来的结果利用规则先进行修正，第二周预测的效果会更好
'''
...
def onlineTest(trade, visit, shop_info, version):
    ...
    np.savetxt('../../data/results/result_' + version + '_2.csv',
               np.concatenate((np.arange(1, 2001).reshape(2000, 1), A), axis=1), delimiter=',',
               fmt='%d')
